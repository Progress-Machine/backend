...
column_names_to_normalize = ["price", "old_price", "celler_rating", "celler_mean_delivery_time",
                             "celler_percent_bad_products",
...
def preprocess_data(data, normalize_data=True):
    ...
    df["description"] = " ".join(
        [morph.parse(word)[0].normal_form for word in df["description"].split() if
    ...
    df["celler_mean_delivery_time"] = float(
        df["celler_mean_delivery_time"][:df["celler_mean_delivery_time"].index("%")]) if df[
                                                                                             "celler_mean_delivery_time"] is not None else 99.3
    df["celler_percent_bad_products"] = float(
        df["celler_percent_bad_products"][:df["celler_percent_bad_products"].index("%")]) if df[
                                                                                                 "celler_percent_bad_products"] is not None else 99.3
    df["sale_percent"] = (df["price"] / df["old_price"]) if df["price"] is not None and df[
        "old_price"] != 0 else 0
    ...
    df["percent_order_of_all_seller"] = (df["order_count"] / df["celler_sold"]) if df[
                                                                                       "order_count"] is not None else 0.03029
    ...
    df.drop(["celler_sold", "comments_link", "celler_working_time", "name",
             "name_comp", "celler_link", "img_link", "text_params", "search_category"],
            inplace=True)

    for feature, mean in zip(
            ["celler_working_time_norm", "old_price", "order_count", "price", "celler_rating"],
    ...
    text_embed_df = pd.DataFrame(data=text_embeddings * 10,
    ...
    if normalize_data:
        ...
        df = df.join(pd.DataFrame(data=text_embeddings * 10,
                                  columns=[f"embed_{i}" for i in range(text_embeddings.shape[1])]))
        return df
    else:
