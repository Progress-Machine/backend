...
def parse_link(link):
    try:
        ...
        try:
            name = product_soup.find("div", class_="product-page__header").find(
                "h1").get_text().strip()
        except:
            name = None
        ...
        # кол-во заказов
        try:
            count_orders = product_soup.find("p", class_='product-order-quantity').find(
                "span").text.strip()
        ...
        try:
            celler_count_products_sold = product_soup.find("li",
                                                           class_="seller-info__item seller-info__item--delivered swiper-slide").find(
                "b", class_="seller-info__value").text.strip()
            celler_count_products_sold = int(
                "".join([i for i in celler_count_products_sold.split() if i.isdigit()]))
        except:
            celler_count_products_sold = None

        try:
            celler_percent_bad_products = product_soup.find("li",
                                                            class_="seller-info__item seller-info__item--defective swiper-slide").find(
                "b", class_="seller-info__value").text.strip()
        except Exception:
            celler_percent_bad_products = None

        try:
            celler_working_time = product_soup.find("li",
                                                    class_="seller-info__item seller-info__item--time swiper-slide").find(
                "b", class_="seller-info__value").text.strip()
        except Exception:
            celler_working_time = None

        try:
            celler_delivery_mean_time = product_soup.find("li",
                                                          class_="seller-info__item seller-info__item--delivery swiper-slide").find(
                "b", class_="seller-info__value").text.strip()
        except AttributeError:
            celler_delivery_mean_time = None

        try:
            celler_rating = float(product_soup.find("span",
                                                    class_="address-rate-mini address-rate-mini--sm").text.strip())
        except Exception:
            celler_rating = None

        try:
            celler_link = product_soup.find("a",
                                            class_="seller-info__name seller-info__name--link").get(
                "href")
        except Exception:
            celler_link = None

        try:
            img_link = product_soup.find("div",
                                         class_="slide__content img-plug j-wba-card-item").find(
                "img").get("src")
        except:
            img_link = None
